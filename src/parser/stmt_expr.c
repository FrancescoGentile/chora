    ...
    if (block == NULL) {
        ...
        if (next_is_expression(p)) {
            ...
            if (block->must_return != NULL) {
                ...
                parse_expression(p, r->expr, &possible);
                possible.free(&possible);
            } else {
                parse_expression(p, r->expr, block->possible);
            }
            ret = get_expr_type(r->expr);
        } else {
            r->expr = NULL;
        ...
        if (block->must_return != NULL && ret != block->must_return) {
            post_type_mismatch(p->eh, MSG_RETURN_TYPE_ERROR2, get_expr_start(r->expr), get_expr_end(r->expr),
...
static void parse_break(Parser *p, ASTBreak *b) {
    ...
    if (block == NULL) {
        ...
        if (next_is_expression(p)) {
            ...
            if (block->must_return != NULL) {
                Vector(TypeRef) possible = create_possible_types(1, block->must_return);
                parse_expression(p, b->expr, &possible);
                possible.free(&possible);
            } else {
                parse_expression(p, b->expr, block->possible);
            }
            ret = get_expr_type(b->expr);
        } else {
            b->expr = NULL;
            ret = get_builtin_type(TYPE_EMPTY);
        }
        ...
        if (block->must_return != NULL && ret != block->must_return) {
            post_error(p->eh, MSG_BREAK_TYPE_ERROR2, get_token_start(&b->break_lex),
...
static void check_block(Parser *p, ASTBlock *b, bool return_value, bool final, usize return_pos) {
    ...
    } else if (!return_value && !block->not_return) {
